...
#     result = ocr_extract(file_bytes, ext)
...
def extract_text_from_pdf(file_bytes: bytes):
    """Extract text using pdfplumber first, then fallback to Tesseract OCR."""
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
...
async def parse_document(file: UploadFile = File(...)):
    file_bytes = await file.read()
    text = extract_text_from_pdf(file_bytes)
    result = DocumentParserAgent().parse_document(text)
    return {"parsed": result}


@app.post("/validate")
async def validate_codes(icd: str, cpt: str):
    result = ClinicalCodingAgent().validate_codes([icd], [cpt])
    return {"coding_validation": result}
...
async def necessity_check(file: UploadFile = File(...)):
    file_bytes = await file.read()
    text = extract_text_from_pdf(file_bytes)
    result = MedicalNecessityAgent().check_necessity(text, "auto", "auto")
    return {"necessity": result}


@app.post("/compliance")
async def compliance_check(payload: dict):
    result = ComplianceAgent().check_compliance(payload)
    return {"compliance": result}


@app.post("/predict")
async def denial_prediction(payload: dict):
    result = DenialPredictorAgent().predict(payload)
    return {"denial_prediction": result}


@app.post("/correct")
async def correct_claim(payload: dict):
    result = CorrectionAgent().correct(payload)
    return {"corrections": result}
